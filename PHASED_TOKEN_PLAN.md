...
- **is_paused**: The operational switch. If `True`, all actions are blocked _but the phase remains the same_.
...
- **Metadata Syncing**: Updates to Project Name, Symbol, and Metadata URI are mirrored directly to the Solana blockchain, ensuring that administrative changes are transparent and auditable by investors.
...
- **`update_project_params`**: Enhanced to sync Name, Symbol, and URI to the blockchain.
- **`Lifecycle Guards`**: Strict on-chain logic that prevents projects from being demoted back to `Draft` and blocks the deletion of active projects to maintain audit trail integrity.
